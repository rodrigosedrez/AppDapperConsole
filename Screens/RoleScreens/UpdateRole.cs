...
namespace Blog.Screens
{
    class UpdateRole
    {
        ...
        {
            Console.Clear();
            Console.WriteLine($"Qual o id do Perfil que vc deseja fazer as modificações:");

            int usuario = int.Parse(Console.ReadLine());
            var repository = new Repository<Role>(connection);
            var elementos = repository.Get(usuario); // pega os elementos do banco a partir do perfil digitado
            Console.Clear();
            Console.WriteLine($"selecione os itens do menu separados apenas por espaço");
            Console.WriteLine($"Nome Slug todos");
            ...
            switch (last)
            {
                case "1": Blog.Screens.UpdateRole.Update(connection); break;
